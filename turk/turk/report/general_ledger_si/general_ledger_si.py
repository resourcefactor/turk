...
def execute(filters=None):
	...
	for r in res:
		if r.get("party_type") and r.get("party"):
			r["party_name"] = frappe.db.get_value(
				r.get("party_type"),
				r.get("party"),
				r.get("party_type").lower() + "_name",
			)
...
def validate_filters(filters, account_details):
	...
	if (
		filters.get("account")
		and filters.get("categorize_by") == "Categorize by Account"
	):
		filters.account = frappe.parse_json(filters.get("account"))
		for account in filters.account:
	...
	if filters.get("voucher_no") and filters.get("categorize_by") in [
...
def get_conditions(filters):
	...
	if filters.get("categorize_by") == "Categorize by Party" and not filters.get(
		"party_type"
	):
	...
	if filters.get("categorize_by") != "Categorize by Voucher (Consolidated)":
		for _acc, acc_dict in gle_map.items():
			...
			if acc_dict.entries:
				...
				if (
					not filters.get("categorize_by") and not filters.get("voucher_no")
				) or (
					filters.get("categorize_by")
					and filters.get("categorize_by") != "Categorize by Voucher"
				):
...
def initialize_gle_map(gl_entries, filters, totals_dict):
	...
	# 1. Collect all voucher types and numbers
	voucher_map = defaultdict(set)
	for gle in gl_entries:
		voucher_map[gle.voucher_type].add(gle.voucher_no)
	# 2. Bulk fetch relevant documents and child tables
	preloaded_docs = {}
	for vtype, vnos in voucher_map.items():
		if vtype in (
			"Sales Invoice",
			"Purchase Invoice",
			"Purchase Receipt",
			...
			# preload main docs just in case, though we don’t use them here
			preloaded_docs[vtype] = frappe.get_all(
				vtype, filters={"name": ["in", doc_names]}, fields=["name"]
			)

			# preload child tables
			if vtype in ("Sales Invoice", "Purchase Invoice", "Purchase Receipt"):
				preloaded_items = frappe.get_all(
					vtype + " Item",
					filters={"parent": ["in", doc_names]},
					fields=["parent", "item_name", "boxes", "qty", "rate", "amount"],
				)
				for item in preloaded_items:
					preloaded_docs.setdefault("items_map", defaultdict(list))
					preloaded_docs["items_map"][item.parent].append(item)

			elif vtype == "Journal Entry":
				preloaded_accounts = frappe.get_all(
					"Journal Entry Account",
					filters={"parent": ["in", doc_names]},
					fields=["parent", "cust_description", "account", "party"],
				)
				for acc in preloaded_accounts:
					preloaded_docs.setdefault("accounts_map", defaultdict(list))
					preloaded_docs["accounts_map"][acc.parent].append(acc)

	# 3. Assign description from preloaded data
	for gle in gl_entries:
		description = ""
		vtype = gle.voucher_type
		vno = gle.voucher_no

		if vtype in ("Sales Invoice", "Purchase Invoice", "Purchase Receipt"):
			items = preloaded_docs.get("items_map", {}).get(vno, [])
			description = "<br/>".join(
				f"{i.item_name}/{i.boxes}/{i.qty}@{i.rate}={i.amount}" for i in items
			)
		elif vtype == "Journal Entry":
			accounts = preloaded_docs.get("accounts_map", {}).get(vno, [])
			description = "<br/>".join(
				i.cust_description for i in accounts if i.cust_description and i.party == gle["party"] and i.account == gle["account"]
			)

		gle["description"] = description

		gle_map.setdefault(
			gle.get(group_by), _dict(totals=copy.deepcopy(totals_dict), entries=[])
		)
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, totals):
	...
	group_by_voucher_consolidated = (
		filters.get("categorize_by") == "Categorize by Voucher (Consolidated)"
	)
...
def get_columns(filters):
	...
	columns += [
		...
		{"label": _("Party Name"), "fieldname": "party_name", "width": 130},
	]
