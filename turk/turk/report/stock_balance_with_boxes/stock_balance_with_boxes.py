...
def execute(filters=None):
    ...
    for sle in sl_entries:
        ...
        if sle.in_qty:
            ...
            sle.in_qty_boxes, sle.in_qty_pieces = calculate_boxes_and_pieces(
                sle.in_qty, item_doc.boxes, item_doc.pieces
            )

        sle.out_qty_boxes = 0
        sle.out_qty_pieces = 0

        if sle.out_qty:
            item_doc = frappe.get_doc("Item", sle.item_code)
            sle.out_qty_boxes, sle.out_qty_pieces = calculate_boxes_and_pieces(
                sle.out_qty, item_doc.boxes, item_doc.pieces
            )

        sle.balance_qty_boxes = 0
        sle.balance_qty_pieces = 0

        if sle.qty_after_transaction:
            item_doc = frappe.get_doc("Item", sle.item_code)
            sle.balance_qty_boxes, sle.balance_qty_pieces = calculate_boxes_and_pieces(
                sle.qty_after_transaction, item_doc.boxes, item_doc.pieces
            )
...
def get_columns():
    columns = [
        ...
        {
            "label": _("In Qty Boxes"),
            "fieldname": "in_qty_boxes",
            "fieldtype": "Int",
            "width": 80,
        },
        {
            "label": _("In Qty Pieces"),
        ...
        {
            "label": _("Out Qty Boxes"),
            "fieldname": "out_qty_boxes",
            "fieldtype": "Int",
            "width": 80,
        },
        {
            "label": _("Out Qty Pieces"),
        ...
        {
            "label": _("Balance Boxes"),
            "fieldname": "balance_qty_boxes",
            "fieldtype": "Int",
            "width": 80,
        },
        {
            "label": _("Balance Pieces"),
            "fieldname": "balance_qty_pieces",
            "fieldtype": "Int",
            "width": 80,
        },
        {
...
#     # Calculate the number of loose pieces based on remaining sqm
#     pieces = int((remaining_sqm / sqm_per_box) * pieces_per_box)

#     return boxes, pieces


# def calculate_boxes_and_pieces(total_sqm, sqm_per_box, pieces_per_box):
#     # Calculate total number of full boxes needed
#     boxes = int(total_sqm // sqm_per_box)

#     # Calculate remaining sqm after full boxes
#     remaining_sqm = total_sqm % sqm_per_box

#     # If there's no remainder, there are no loose pieces
#     if remaining_sqm == 0:
#         pieces = 0
#     else:
#         # Adjust if there’s any fractional remainder, add one extra box
#         boxes += 1
#         pieces = 0  # Since we are accommodating all in boxes, loose pieces become zero

#     return boxes, pieces


# def calculate_boxes_and_pieces(total_sqm, sqm_per_box, pieces_per_box):
#     # Calculate total number of full boxes
#     boxes = int(total_sqm // sqm_per_box)
...
#     # Calculate the number of loose pieces based on remaining sqm
#     pieces = (
#         int((remaining_sqm / sqm_per_box) * pieces_per_box) if remaining_sqm > 0 else 0
#     )

#     # If remaining sqm requires an additional box (e.g., loose pieces fit into one more box), increase box count and reset pieces
